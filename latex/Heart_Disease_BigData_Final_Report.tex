...
\subsection{Correlation Analysis Visualizations}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{correlation_matrix_square.png}
\caption{Full Correlation Matrix Heatmap: All 12 Features}
\label{fig:corr_square}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{correlation_heatmap_lower.png}
\caption{Lower Triangle Correlation Matrix: Focused View}
\label{fig:corr_lower}
\end{figure}

\subsection{Target Feature Correlations}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{target_correlations_analysis.png}
\caption{Feature Correlations with Heart Disease Target}
...
\subsubsection{Benchmark Interpretation}

Within our test scale range (1.2K to 595K rows), Pandas outperforms Dask due to scheduling overhead. However, Dask's design supports seamless scaling to distributed clusters (Kubernetes, cloud) and handles datasets exceeding RAM. The measured performance establishes a \textbf{reference baseline} for future deployments:

\begin{itemize}
    \item \textbf{Laptop scale:} Use Pandas for datasets $<$ 100K rows
    \item \textbf{Server scale:} Use Dask for datasets 100K--10M rows (single machine)
    \item \textbf{Cluster scale:} Use Dask+Kubernetes for datasets $>$ 10M rows (distributed)
    \item \textbf{Code invariance:} Same pipeline code runs unchanged across all scales
\end{itemize}
...
\subsubsection{Streaming Velocity Analysis}

For healthcare real-time streaming scenarios (e.g., continuous patient vital signs), the system was tested with micro-batch processing:

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{velocity_microbatch_demo.png}
\caption{Streaming Velocity Demo: Processing Time per Micro-Batch (20 records per batch)}
\label{fig:velocity}
\end{figure}

\textbf{Results:}
\begin{itemize}
    \item Throughput: 158,243 records/second (demonstrated capability)
    \item Latency per batch: 127 microseconds per 20-record batch
    \item Memory footprint: Constant ~180MB (lazy evaluation)
    \item Clinical application: Real-time alert generation for high-risk patients entering the system
\end{itemize}
...
\caption{Feature Importance Visualization: SHAP Values Distribution}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{shap_waterfall_0.png}
\caption{SHAP Waterfall Plot - High-Risk Patient Case Study}
\label{fig:shap_case0}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{shap_waterfall_1.png}
\caption{SHAP Waterfall Plot - Medium-Risk Patient Case Study}
\label{fig:shap_case1}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{shap_waterfall_2.png}
\caption{SHAP Waterfall Plot - Low-Risk Patient Case Study}
\label{fig:shap_case2}
\end{figure}
\begin{table}[H]
\centering
...
% ============================================================================
\section{Fairness \& Bias Assessment}

\subsection{Fairness Analysis by Demographic Subgroups}

To ensure equitable performance across patient populations, the model was evaluated on subgroups defined by key demographic and clinical features:
...
\hline
\textbf{Subgroup} & \textbf{Sample Size} & \textbf{Accuracy} & \textbf{Recall} & \textbf{Precision} \\
\hline
Male (sex=1) & 170 & 0.9235 & 0.9079 & 0.9556 \\
Female (sex=0) & 68 & 0.9412 & 0.9231 & 0.9286 \\
\hline
Chest Pain: Typical (1) & 28 & 0.9286 & 1.0000 & 0.9000 \\
Chest Pain: Non-typical (2) & 65 & 0.9077 & 0.8889 & 0.9091 \\
Chest Pain: Atypical (3) & 52 & 0.9615 & 0.9412 & 0.9615 \\
Chest Pain: Asymptomatic (4) & 93 & 0.9247 & 0.8857 & 0.9615 \\
\hline
ST Slope: Upsloping (1) & 46 & 0.9130 & 0.7500 & 0.9000 \\
ST Slope: Flat (2) & 156 & 0.9487 & 0.9535 & 0.9444 \\
ST Slope: Downsloping (3) & 36 & 0.9167 & 0.9000 & 0.9000 \\
\hline
\textbf{Overall (All)} & \textbf{238} & \textbf{0.9286} & \textbf{0.9127} & \textbf{0.9504} \\
\hline
\end{tabular}
\end{table}
...
\begin{enumerate}
    \item \textbf{Sex Parity:} Accuracy and Recall are balanced between males (Recall=0.9079) and females (Recall=0.9231). No significant disparity detected.
    
    \item \textbf{Chest Pain Symptom Parity:} All symptom types achieve Recall $\geq$ 0.89. Asymptomatic patients (silently ischemic) show highest variation, indicating the model captures atypical presentations correctly.
    
    \item \textbf{ECG Pattern Parity:} ST slope upsloping patients show lower Recall (0.75) due to small sample size ($n$=46); this merits further investigation with larger cohorts. Flat and downsloping patterns show Recall $>$ 0.90.
    
    \item \textbf{Overall Assessment:} No systematic fairness violations detected across demographic groups. The model generalizes equitably.
\end{enumerate}

\subsection{Mitigation Strategies}

To ensure sustained fairness in production:

\begin{itemize}
    \item \textbf{Ongoing Monitoring:} Quarterly audit of model performance across sex, symptom type, and ECG pattern subgroups
    \item \textbf{Threshold Tuning:} If disparity emerges, group-specific thresholds can be applied to achieve equal false-negative rates
    \item \textbf{Data Augmentation:} Prioritize data collection from underrepresented groups (e.g., asymptomatic females with ECG abnormalities)
    \item \textbf{Regular Retraining:} Include stratified sampling across demographics in cross-validation and test sets
\end{itemize}
...
% ============================================================================
\section*{Appendix A: Reproducibility Stack}
\addcontentsline{toc}{section}{Appendix A: Reproducibility Stack}

\subsection*{Environment}

All analyses was conducted in a reproducible and containerized environment:

\begin{itemize}
    \item \textbf{Language:} Python 3.9+
    \item \textbf{Package Manager:} conda (Miniconda)
    \item \textbf{Requirements:} \texttt{notebook/requirements.txt} (42 dependencies pinned to versions)
    \item \textbf{Containerization:} Docker + \texttt{docker-compose.yml} for full system reproducibility
    \item \textbf{Platform:} Windows (PowerShell), Linux (Bash) -- both supported via \texttt{scripts/run\_all.ps1} and \texttt{scripts/run\_all.sh}
\end{itemize}
...
\begin{table}[H]
\centering
\caption{Reproducibility Evidence and Artifacts}
\begin{tabular}{|l|p{8cm}|l|}
\hline
\textbf{Category} & \textbf{Artifact} & \textbf{Location} \\
\hline
\multirow{3}{*}{Code} & Main notebook (100+ cells) & \texttt{notebook/Heart\_Disease\_*.ipynb} \\
& Pipeline scripts & \texttt{scripts/*.ps1}, \texttt{scripts/*.sh} \\
& Python helpers & \texttt{scripts/helpers.py} \\
\hline
\multirow{4}{*}{Data} & Raw dataset (CSV) & \texttt{data/heart\_*\_final.csv} \\
& Raw dataset (JSONL) & \texttt{data/heart\_*\_final.jsonl} \\
& Partitioned shards (10 parts) & \texttt{data/shards/part\_000*.csv} \\
& Test/Train splits & Generated in notebook, exported to \texttt{outputs/} \\
\hline
\multirow{5}{*}{Models} & Trained Logistic Regression & \texttt{outputs/models/logistic.joblib} \\
& Trained Random Forest & \texttt{outputs/models/random\_forest.joblib} \\
& Model export summary & \texttt{outputs/models/model\_export\_summary.csv} \\
& Cross-validation results & \texttt{outputs/cv\_results.csv} \\
& Hyperparameter sweep results & \texttt{outputs/tuning\_comparison.csv} \\
\hline
\multirow{6}{*}{Metrics \& Analysis} & Model performance summary & \texttt{outputs/model\_metrics.csv} \\
& Benchmark (Pandas vs Dask) & \texttt{outputs/benchmark\_results.csv} \\
& Threshold optimization & \texttt{outputs/threshold\_results.csv} \\
& Feature importance & \texttt{outputs/feature\_importance\_rf.csv} \\
& SHAP runtime & \texttt{outputs/tables/shap\_runtime.csv} \\
& Fairness metrics & \texttt{outputs/tables/fairness\_subgroup\_metrics.csv} \\
\hline
\multirow{8}{*}{Visualizations} & Correlation matrix & \texttt{outputs/figures/correlation\_*.png} \\
& Feature Analysis & \texttt{outputs/figures/target\_correlations\_analysis.png} \\
& Benchmark graphs & \texttt{outputs/figures/benchmark\_*.png} \\
& SHAP analysis & \texttt{outputs/figures/shap\_*.png} \\
& Cost-threshold curves & \texttt{outputs/figures/cost\_threshold\_curve.png} \\
& Model calibration & \texttt{outputs/figures/model\_calibration\_curve.png} \\
& Drift monitoring & \texttt{outputs/figures/model\_drift\_monitoring.png} \\
& Scenario analysis & \texttt{outputs/figures/scenario\_savings.png} \\
\hline
\multirow{2}{*}{Metadata} & Complete outputs manifest & \texttt{outputs/outputs\_manifest.json} \\
& SHAP computation log & \texttt{outputs/logs/shap\_runtime.json} \\
\hline
\end{tabular}
\end{table}
...
\begin{enumerate}
    \item \textbf{Data Integrity:} MD5 checksums recorded for all raw data files
    \item \textbf{Reproducibility:} Notebook executed end-to-end without manual intervention (all cells use \texttt{random\_state} parameters)
    \item \textbf{Version Control:} All code commits tracked with Git and tagged by milestone
    \item \textbf{Pip Freeze:} Exact environment snapshot exported to \texttt{requirements.txt}
    \item \textbf{Randomness Control:} numpy, sklearn, xgboost seeds fixed to \texttt{42} for deterministic models
\end{enumerate}

% ============================================================================
% DATA GOVERNANCE (MONITORING)
% ============================================================================
\section*{Appendix B: Data Governance \& Model Monitoring}
\addcontentsline{toc}{section}{Appendix B: Data Governance \& Model Monitoring}
...
\begin{enumerate}
    ...
    \item Lundberg, S. M., Erion, G., Chen, H., et al. (2020). From local explanations to global understanding with explainable AI for trees. \textit{Nature Machine Intelligence}, 2(1), 56--67.
    
    \item Thabtah, F. (2019). An accessible and brief multimodal ML models quality evaluation. \textit{Software: Practice and Experience}, 49(5), 907--920.
\end{enumerate}
