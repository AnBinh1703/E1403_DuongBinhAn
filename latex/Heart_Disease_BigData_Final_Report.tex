...
\begin{itemize}
    ...
    \begin{itemize}
        \item Near-balanced target (639 disease / 553 healthy $\approx$ 53.6\% / 46.4\%)
...
\hline
Velocity & Real-time patient monitoring & Micro-batch streaming simulation (152,921 rec/sec) \\
\hline
...
172 cholesterol values (14.45\%) were recorded as 0, which is biologically impossible. These were treated as missing data and imputed using \textbf{median imputation} (median = 223 mg/dL). Median was chosen over mean to avoid distortion from outliers in the cholesterol distribution.
...
\textbf{Treatment:} Winsorization (capping at 5th/95th percentiles) was applied rather than deletion, preserving all 1,190 patient records while mitigating extreme value influence.

\subsection{Scaling and Encoding}

\begin{itemize}
    \item \textbf{StandardScaler:} Applied to 8 numerical features (zero mean, unit variance)
    \item \textbf{One-Hot Encoding:} Applied to 3 categorical features (Age\_Group, BP\_Category, Cholesterol\_Risk\_Level)
    \item \textbf{Final feature count:} 22 features after transformation
\end{itemize}
...
\begin{enumerate}
    \item \textbf{ST slope} shows the strongest correlation with heart disease ($r = 0.51$), consistent with known clinical significance in indicating myocardial ischemia.
    \item \textbf{Chest pain type} ($r = 0.43$) and \textbf{oldpeak} ($r = 0.40$) are also strongly correlated with the target.
...
\texttt{RandomizedSearchCV} (20 iterations, 3-fold stratified CV, scoring = recall) was applied to both Random Forest and XGBoost:
...
\hline
Logistic Regression & 0.8487 & 0.8631 & 0.8492 & 0.8654 & 0.9112 \\
Random Forest (Initial) & 0.9244 & 0.9355 & 0.9206 & 0.9280 & 0.9664 \\
XGBoost (Initial) & 0.9202 & 0.9286 & 0.9127 & 0.9260 & 0.9636 \\
Random Forest (Tuned) & 0.9286 & 0.9431 & 0.9206 & 0.9317 & 0.9725 \\
...
\begin{table}[H]
\centering
\caption{Cross-Validation Performance (Mean $\pm$ Std)}
\begin{tabular}{|l|c|c|c|}
\hline
\textbf{Model} & \textbf{CV Accuracy} & \textbf{CV ROC-AUC} & \textbf{CV Recall} \\
\hline
Logistic Regression & 0.8498 $\pm$ 0.0193 & 0.9131 $\pm$ 0.0163 & 0.8730 $\pm$ 0.0248 \\
Random Forest & 0.9117 $\pm$ 0.0192 & 0.9614 $\pm$ 0.0102 & 0.9112 $\pm$ 0.0150 \\
XGBoost & 0.9227 $\pm$ 0.0142 & 0.9619 $\pm$ 0.0059 & 0.9300 $\pm$ 0.0202 \\
\hline
\end{tabular}
\end{table}
...
\begin{itemize}
    \item Random Forest (Tuned): TN=105, FP=7, FN=10, TP=116
...
\begin{itemize}
    \item Logistic Regression: 0.1146
    \item Random Forest (Tuned): 0.0697
...
\begin{table}[H]
\centering
\caption{Pandas vs.\ Dask Scaling Performance}
\begin{tabular}{|c|r|c|c|c|}
\hline
\textbf{Scale} & \textbf{Rows} & \textbf{Pandas (s)} & \textbf{Dask (s)} & \textbf{Speedup} \\
\hline
1$\times$ & 1,190 & 0.019 & 0.188 & 0.10$\times$ \\
10$\times$ & 11,900 & 0.016 & 0.154 & 0.11$\times$ \\
50$\times$ & 59,500 & 0.028 & 0.235 & 0.12$\times$ \\
100$\times$ & 119,000 & 0.048 & 0.349 & 0.14$\times$ \\
500$\times$ & 595,000 & 0.270 & 1.650 & 0.16$\times$ \\
\hline
\end{tabular}
\end{table}

\textbf{Observation:} For datasets $<$100K rows, Pandas is faster due to Dask's scheduling overhead. Dask's advantages emerge at larger scales (millions of rows) where:
\begin{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Robust ML pipeline:} Complete preprocessing (median imputation, IQR/Z-score outlier treatment, winsorization, 7 engineered features, StandardScaler + OneHotEncoder) feeding three classifiers with cross-validation and hyperparameter tuning.

    \item \textbf{Strong performance:} Random Forest achieves Accuracy = 0.9244, Recall = 0.9206, AUC = 0.9664. Cost-optimized threshold raises Recall to 0.992 with 88.1\% cost reduction.
