...
\textbf{Feature selection:} All 20 features were retained for modeling. The engineered \texttt{Heart\_Risk\_Index} proved to be the single most predictive feature in Random Forest, validating the domain-driven approach. Both Random Forest and XGBoost importance rankings were used to confirm feature relevance (no redundant features identified).
...
\begin{enumerate}
    ...
    \item \textbf{Leak-free ML pipeline:} Four custom \texttt{sklearn} transformers (\texttt{CholesterolFixer}, \texttt{DataFrameImputer}, \texttt{DataFrameWinsorizer}, \texttt{FeatureEngineer}) followed by a \texttt{ColumnTransformer} (StandardScaler for 16 numeric + OneHotEncoder for 2 categorical features) are wrapped inside a scikit-learn \texttt{Pipeline}, ensuring all fitted parameters (medians, percentile bounds, scaler/encoder) are learned on training data only.  Cross-validation and hyperparameter tuning use Pipeline-wrapped models to prevent data leakage.  Threshold selection uses out-of-fold probabilities (\texttt{cross\_val\_predict}) on the training set.
