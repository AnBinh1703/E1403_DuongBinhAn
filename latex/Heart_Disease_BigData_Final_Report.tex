...
\subsubsection{Data Variety: Multi-Format Loading}
To demonstrate Variety, data was loaded from both CSV and JSON Lines formats:
\begin{itemize}
    \item \textbf{CSV Format:} Traditional tabular format for structured clinical data
    \item \textbf{JSON Lines Format:} NoSQL-style format common in streaming pipelines
\end{itemize}

\subsubsection{Data Velocity: Micro-Batch Processing}
A micro-batch streaming simulation was implemented to demonstrate real-time processing capabilities:
\begin{itemize}
    \item Batch size: 100 records (configurable)
    \item Processing latency: <5ms per batch
    \item Compatible with production streaming frameworks (Kafka, Spark Streaming)
\end{itemize}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{velocity_microbatch_demo.png}
\caption{Micro-Batch Streaming Simulation Output}
\label{fig:velocity}
\end{figure}
...
\hline
Velocity & Real-time monitoring & Micro-batch streaming simulation \\
\hline
Variety & Mixed data types & CSV + JSON Lines multi-format loading \\
\hline
\end{tabular}
\end{table}

\subsection{Big Data Pipeline Architecture}

Figure~\ref{fig:pipeline} illustrates the complete end-to-end Big Data architecture implemented in this project.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{architecture_pipeline.png}
\caption{Big Data Analytics Pipeline Architecture for Heart Disease Prediction}
\label{fig:pipeline}
\end{figure}
...
\subsection{Statistical Validation with Effect Sizes}

Rigorous statistical testing was performed to validate observed differences between disease groups. Results include both p-values and effect size measures for scientific rigor.

\begin{table}[H]
\centering
\caption{Statistical Validation of Risk Factors (Numerical \& Categorical)}
\label{tab:statval}
\input{tables/stat_tests.tex}
\end{table}

\textbf{Interpretation:} All features show statistically significant differences (p < 0.05) between disease groups. ST slope and chest pain type exhibit the largest effect sizes (Cramer's V > 0.4), confirming their importance as predictors.
...
Standard classification uses 0.5 as the decision threshold. However, the asymmetric costs of false negatives (missed disease: \$20,000) versus false positives (unnecessary screening: \$200) require threshold optimization.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{cost_threshold_curve.png}
\caption{Cost-Sensitive Threshold Optimization Analysis}
\label{fig:threshold}
\end{figure}

The optimal threshold was determined to be approximately 0.35, which reduces total expected cost by prioritizing recall while maintaining acceptable precision.

\subsection{Model Calibration Analysis}

Well-calibrated probabilities are essential for clinical decision-making. Reliability diagrams (calibration curves) compare predicted probabilities against observed frequencies.

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{model_calibration_curve.png}
\caption{Calibration Curves and Brier Score Analysis}
\label{fig:calibration}
\end{figure}

\textbf{Key Findings:}
\begin{itemize}
    \item Logistic Regression: Best calibrated (Brier Score lowest)
    \item XGBoost: Slightly overconfident in predictions
    \item All models suitable for clinical deployment with appropriate calibration
\end{itemize}
...
\subsection{Detailed Runtime Benchmark: Pandas vs Dask}

A comprehensive benchmark comparison was conducted to quantify the performance benefits of Dask over traditional Pandas for various data operations.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{benchmark_pandas_vs_dask.png}
\caption{Pandas vs Dask Runtime Benchmark Comparison}
\label{fig:benchmark}
\end{figure}

\textbf{Key Observations:}
\begin{itemize}
    \item Dask shows superior performance for aggregation operations at scale
    \item GroupBy operations benefit most from parallelization
    \item Memory efficiency allows Dask to handle larger-than-RAM datasets
\end{itemize}
...
\subsection{SHAP (SHapley Additive exPlanations) Analysis}

Beyond traditional feature importance, SHAP values were computed using TreeExplainer on the best Random Forest model. SHAP provides a unified measure of feature contributions grounded in cooperative game theory.

\textbf{Key Advantages of SHAP:}
\begin{itemize}
    \item \textbf{Local Interpretability:} Explains individual patient predictions, enabling clinicians to understand \emph{why} a specific patient was flagged.
    \item \textbf{Global Interpretability:} Summary plots aggregate per-feature contributions across the entire cohort.
    \item \textbf{Consistency:} SHAP satisfies desirable theoretical properties (additivity, symmetry, null player).
\end{itemize}

\textbf{SHAP Summary Findings:}
\begin{itemize}
    \item ST slope and chest pain type exhibit the largest mean absolute SHAP values, confirming their dominant role.
    \item High values of oldpeak and exercise angina consistently push predictions toward higher disease probability.
    \item Max heart rate shows a protective (negative SHAP) effect when high, aligning with medical literature.
\end{itemize}
...
\subsection{Scenario-Based Cost-Benefit Analysis}

Three scenarios (Worst Case, Expected Case, Best Case) were analyzed to provide risk-adjusted investment projections.

\begin{table}[H]
\centering
\caption{Scenario-Based Cost-Benefit Analysis}
\label{tab:scenario}
\input{tables/scenario_cost_benefit.tex}
\end{table}
...
\textbf{Executive Summary:} Even under worst-case assumptions, the system delivers positive returns, making this a low-risk, high-reward investment.

%==============================================================================
\section{Data Governance and Model Monitoring}
%==============================================================================

\subsection{Model Drift Detection}

Continuous monitoring for distribution drift is essential for maintaining model performance in production. The Kolmogorov-Smirnov test is used to detect significant shifts in prediction distributions.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{model_drift_monitoring.png}
\caption{Model Drift Detection and Performance Monitoring}
\label{fig:drift}
\end{figure}

\subsection{Retraining Schedule}

\begin{table}[H]
\centering
\caption{Model Retraining Trigger Matrix}
\label{tab:retrain}
\input{tables/retraining_schedule.tex}
\end{table}
...
\label{tab:kpis}
\input{tables/monitoring_kpis.tex}
\end{table}

\subsection{Governance Framework}

\begin{enumerate}
    \item \textbf{Data Quality:} Automated validation pipelines with schema enforcement
    \item \textbf{Data Security:} PHI/PII encryption (AES-256), RBAC, HIPAA compliance
    \item \textbf{Model Governance:} Version control, A/B testing, shadow deployment
    \item \textbf{Compliance \& Ethics:} Bias monitoring, explainability reports, fairness audits
\end{enumerate}
...
\begin{enumerate}
    ...
    \item World Health Organization. (2021). Cardiovascular diseases (CVDs) Fact Sheet. WHO Media Centre. Retrieved from https://www.who.int/news-room/fact-sheets/detail/cardiovascular-diseases
